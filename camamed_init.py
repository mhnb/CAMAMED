...
subprocess.check_call('tar -xvf MetaPhlAn-2.7.8.tar.gz', shell=True)
subprocess.check_call('rm MetaPhlAn-2.7.8.tar.gz', shell=True)
subprocess.check_call('mv ./metaphlan_databases ./MetaPhlAn-2.7.8', shell=True)
subprocess.check_call('rm FastQC.tar.gz', shell=True)
os.system('clear')
...
subprocess.check_call('chmod +x ./MetaPhlAn-2.7.8/metaphlan2.py', shell=True)
...
exist = subprocess.call('command -v bowtie2 --version >> /dev/null', shell=True)
if exist !=0:
	print("installing bowtie2")
	subprocess.check_call('sudo apt-get install -y bowtie2', shell=True)
...
if sys.version_info.major==3:
	...
	#exist = importlib.util.find_spec('pip')
	#if exist is None:
